...
class CustomAccountManager(BaseUserManager):
    def create_user(self, email, first_name, last_name, password, **other_fields):
        ...
        if not email:
            raise ValueError(_('Teclea una cuenta de correo valida'))
    ...
    def create_superuser(self, email, first_name, last_name, password, **other_fields):
        ...
        if other_fields.get('is_staff') is not True:
            raise ValueError('Si es de staff debe ser un Superusuario')
        if other_fields.get('is_superuser') is not True:
            raise ValueError('si se esta creando un Superusuario debe asignarse como verdadero is_superuser')
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    def count_posts(self):
        return Paginas.objects.filter(autor=self).count()
